...
export default function NovaPropostaPage() {
  ...
  if (fetchingEditais)
    return (
      <div className="card">
        <p className="p">Carregando formulário...</p>
      </div>
    );
  ...
  return (
    <div className="grid" style={{ gap: 14 }}>
      <div className="card">
        ...
        <p className="p">
          Este formulário é propositalmente exigente. A meta é impedir proposta genérica e facilitar
          avaliação objetiva: problema → solução → equipe → entregas mensais → evidências → impacto.
        </p>

        {/* LEI banner */}
        <a href="/LEI.pdf" target="_blank" rel="noopener noreferrer" className="lei-banner" style={{ marginBottom: 14 }}>
          📋 <strong>Consulte a Lei Municipal de Inovação</strong> — Leitura recomendada antes de preencher a proposta
        </a>

        {scoreHints.length > 0 && (
          <div className="card" style={{ padding: 14, borderColor: "var(--warn)" }}>
            <div className="badge">
              <strong>Guardrails</strong> problemas encontrados
            </div>
        ...
        <div className="grid two" style={{ marginTop: 14 }}>
          ...
          <div className="row">
            ...
            <input
              ...
              placeholder="ex.: educação digital, saúde, turismo..."
            />
          </div>
        </div>

        <div className="row" style={{ marginTop: 14 }}>
          <div className="label">Duração (meses) — o cronograma abaixo será ajustado automaticamente</div>
          <input
            className="input"
            type="number"
        ...
        <div
          ...
          style={{
            padding: 14,
            marginTop: 14,
            borderColor: Math.abs(rateioSum - 100) > 0.01 && rateioSum !== 0 ? "var(--warn)" : "var(--border)",
          }}
        >
          <div className="badge">
            <strong>Equipe do Projeto</strong> {rateioSum}% da bolsa distribuída
          </div>
          <p className="p" style={{ marginTop: 10, fontSize: 13 }}>
            Adicione todos os membros. A soma do rateio da bolsa deve dar 100% ou 0%. Se houver algum
            menor de idade (&lt; 18 anos), marque a caixa e preencha os dados do responsável legal. O
            primeiro membro é o proponente.
          </p>
              ...
              <div className="card" key={idx} style={{ padding: 14 }}>
                <div className="grid two">
                  ...
                  <div className="row">
                    ...
                    <input
                      className="input"
                      value={it.cpf}
                      onChange={(e) => updateEq(idx, { cpf: e.target.value })}
                    />
                  </div>
                </div>
                ...
                <label style={{ display: "flex", gap: 10, alignItems: "center", marginTop: 10 }}>
                  <input
                    type="checkbox"
                    checked={it.ehMenor}
                    onChange={(e) => updateEq(idx, { ehMenor: e.target.checked })}
                  />
                  <span className="p" style={{ margin: 0 }}>
                    Membro é menor de 18 anos
                  </span>
                </label>
                ...
                {it.ehMenor && (
                  <div
                    className="grid two"
                    style={{ marginTop: 10, padding: 10, background: "rgba(255,255,255,0.02)", borderRadius: 10 }}
                  >
                    <div className="row">
                      <div className="label">Nome do Responsável Legal</div>
                      <input
                        className="input"
                        value={it.responsavelLegal}
                        onChange={(e) => updateEq(idx, { responsavelLegal: e.target.value })}
                      />
                    </div>
                    <div className="row">
                      ...
                      <input
                        className="input"
                        value={it.cpfResponsavel}
                        onChange={(e) => updateEq(idx, { cpfResponsavel: e.target.value })}
                      />
                    </div>
                  </div>
                )}
          ...
          <div style={{ marginTop: 12, display: "flex", gap: 10 }}>
            <button className="btn secondary" onClick={addMembro} type="button">
              Adicionar membro
            </button>
          </div>
        </div>
        ...
        <div className="grid two" style={{ marginTop: 14 }}>
          <div className="row">
            ...
            <RichEditor
              ...
              placeholder="Qual é o gargalo hoje? Quem sofre? Como você sabe que isso é real?"
            />
          </div>
          <div className="row">
            ...
            <RichEditor
              ...
              placeholder="Quem vai usar? Onde? Com quais limitações?"
            />
          </div>
        </div>
        ...
        <div className="grid two" style={{ marginTop: 14 }}>
          <div className="row">
            ...
            <RichEditor value={state.propostaValor} onChange={(v) => set("propostaValor", v)} />
          </div>
          <div className="row">
            <div className="label">Solução (o que você vai construir e o que NÃO vai)</div>
            <RichEditor value={state.solucao} onChange={(v) => set("solucao", v)} />
          </div>
        </div>
        ...
        <div className="grid two" style={{ marginTop: 14 }}>
          <div className="row">
            ...
            <RichEditor value={state.metodologia} onChange={(v) => set("metodologia", v)} />
          </div>
          <div className="row">
            <div className="label">Viabilidade (recursos, acessos, dependências)</div>
            <RichEditor value={state.viabilidade} onChange={(v) => set("viabilidade", v)} />
          </div>
        </div>
        ...
        <div className="grid two" style={{ marginTop: 14 }}>
          <div className="row">
            ...
            <RichEditor value={state.riscos} onChange={(v) => set("riscos", v)} />
          </div>
          <div className="row">
            <div className="label">Indicadores e critérios de sucesso (com número)</div>
            <RichEditor value={state.indicadores} onChange={(v) => set("indicadores", v)} />
          </div>
        </div>
        ...
        <div className="grid two" style={{ marginTop: 14 }}>
          <div className="row">
            ...
            <RichEditor value={state.orcamentoRateio} onChange={(v) => set("orcamentoRateio", v)} />
          </div>
          <div className="row">
            <div className="label">Página pública: como mostrar evolução sem expor segredos/dados</div>
            <RichEditor value={state.paginaPublicaPlano} onChange={(v) => set("paginaPublicaPlano", v)} />
          </div>
        </div>

        {/* ========================================================= */}
        {/* CRONOGRAMA — auto-synced with duracaoMeses */}
        {/* ========================================================= */}
        <div className="card" style={{ padding: 14, marginTop: 14 }}>
          <div className="badge">
            <strong>Cronograma mensal</strong> {state.duracaoMeses} mês(es) • entregáveis + evidências
          </div>
          <p className="p" style={{ marginTop: 10 }}>
            Os meses são gerados automaticamente conforme a duração informada acima. Para cada mês: 1)
            entregável verificável, 2) evidência anexável/publicável, 3) critério de aceitação.
          </p>
          <div className="grid" style={{ gap: 12 }}>
            {state.cronograma.map((it, idx) => (
              <div className="card" key={idx} style={{ padding: 14 }}>
                <div className="badge">
                  <strong>Mês {it.mes}</strong>
                </div>
                <div className="grid two" style={{ marginTop: 10 }}>
                  <div className="row">
                    ...
                    <input
                      className="input"
                      value={it.entregavel}
                      onChange={(e) => updateCron(idx, { entregavel: e.target.value })}
                      placeholder="ex.: protótipo funcional + manual curto"
                    />
                  </div>
                  <div className="row">
                    ...
                    <input
                      className="input"
                      value={it.evidencia}
                      onChange={(e) => updateCron(idx, { evidencia: e.target.value })}
                      placeholder="ex.: link demo, vídeo, prints, documento"
                    />
                  </div>
                </div>
                <div className="row" style={{ marginTop: 10 }}>
                  ...
                  <input
                    className="input"
                    value={it.criterioAceitacao}
                    onChange={(e) => updateCron(idx, { criterioAceitacao: e.target.value })}
                    placeholder="ex.: 1 escola testando + checklist X aprovado"
                  />
                </div>
              </div>
            ))}
        ...
        <div className="card" style={{ padding: 14, marginTop: 14 }}>
          <div className="badge">
            <strong>Proposta Completa em PDF</strong> <span style={{ color: "var(--muted)" }}>— OPCIONAL</span>
          </div>
          <p className="p" style={{ marginTop: 10, fontSize: 13 }}>
            Caso deseje, anexe o PDF completo da sua proposta. Isso é <strong>totalmente opcional</strong> e
            serve apenas como material complementar para a avaliação.
          </p>
          <label
          ...
          >
            ...
            />
            {pdfUploading ? (
              <span className="p" style={{ margin: 0 }}>Enviando PDF...</span>
            ) : pdfFile ? (
              <>
                <span style={{ fontSize: 24 }}>✅</span>
                <span className="p" style={{ margin: 0 }}>{pdfFile.name} — enviado com sucesso</span>
              </>
            ) : (
              <>
                <span style={{ fontSize: 28 }}>📄</span>
                <span className="p" style={{ margin: 0, fontWeight: 600 }}>Clique para selecionar PDF</span>
                <span className="p" style={{ margin: 0, fontSize: 12 }}>Máximo 10 MB</span>
              </>
            )}
          </label>
        </div>
        ...
        <label style={{ display: "flex", gap: 10, alignItems: "center", marginTop: 14 }}>
          <input
            type="checkbox"
            checked={state.ipConcorda}
            onChange={(e) => set("ipConcorda", e.target.checked as any)}
          />
          <span className="p" style={{ margin: 0 }}>
            Declaro ciência sobre propriedade intelectual do Município e regras de confidencialidade
            (Anexo II).{" "}
            <a href="/LEI.pdf" target="_blank" rel="noopener noreferrer" style={{ color: "var(--accent)", textDecoration: "underline" }}>
              Consulte a Lei
            </a>
          </span>
        </label>
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 14, alignItems: "center" }}>
          <button className="btn" onClick={submit} disabled={loading || editais.length === 0}>
            {loading ? "Enviando..." : "Enviar proposta"}
          </button>
          {msg && (
            <span className="p" style={{ margin: 0, color: "var(--bad)" }}>
              {msg}
            </span>
          )}
        </div>
